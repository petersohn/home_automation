...
enum JsonVariantType {
    ...
    JSON_NEGATIVE_INTEGER,  // JsonVariant stores an JsonUInt that must be
                            // negated
...
struct StringTraits<const __FlashStringHelper*, void> {
    ...
    typedef const char* duplicate_t;
    template <typename Buffer>
    static duplicate_t duplicate(
        const __FlashStringHelper* str, Buffer* buffer) {
        if (!str) {
            return NULL;
        }
...
public:
    ...
    friend bool operator==(
        const JsonVariantComparisons& variant, TComparand comparand) {
    ...
    template <typename TComparand>
    friend bool operator<=(
        const JsonVariantComparisons& left, TComparand right) {
        return left.as<TComparand>() <= right;
    }
    template <typename TComparand>
    friend bool operator<=(
        TComparand comparand, const JsonVariantComparisons& variant) {
    ...
    template <typename TComparand>
    friend bool operator<(
        const JsonVariantComparisons& varian, TComparand comparand) {
...
private:
    ...
    template <typename TComparand>
    typename EnableIf<
        !IsVariant<TComparand>::value && !StringTraits<TComparand>::has_equals,
        bool>::type
    ...
    bool equals(const JsonVariantComparisons<TVariant2>& right) const {
        ...
        if (is<char*>() && right.template is<char*>()) {
            return StringTraits<const char*>::equals(
                as<char*>(), right.template as<char*>());
        }
        return false;
    }
};
...
public:
    ...
    FORCE_INLINE typename EnableIf<
        StringTraits<TString>::has_equals,
    ...
    template <typename TString>
    FORCE_INLINE typename EnableIf<
        StringTraits<const TString*>::has_equals,
        JsonObjectSubscript<const TString*> >::type
    operator[](const TString* key) {
        return impl()->template as<JsonObject>()[key];
    }
...
public:
    ...
    template <typename T>
    JsonVariant(
        T value, typename Internals::EnableIf<
    ...
    JsonVariant(
        T value, typename Internals::EnableIf<
                     Internals::IsSignedIntegral<T>::value ||
    ...
    template <typename T>
    typename Internals::EnableIf<
        Internals::IsSame<T, const char*>::value ||
    ...
    template <typename T>
    typename Internals::EnableIf<
        Internals::StringTraits<T>::has_append, T>::type
    as() const {
    ...
    typename Internals::EnableIf<
        Internals::IsSame<
            typename Internals::RemoveReference<T>::type, JsonArray>::value,
        JsonArray&>::type
    as() const {
    ...
    typename Internals::EnableIf<
        Internals::IsSame<
            typename Internals::RemoveReference<T>::type,
    ...
    typename Internals::EnableIf<
        Internals::IsSame<
            typename Internals::RemoveReference<T>::type, JsonObject>::value,
        JsonObject&>::type
    as() const {
    ...
    typename Internals::EnableIf<
        Internals::IsSame<
            typename Internals::RemoveConst<
...
public:
    ...
    typename Internals::EnableIf<
        !Internals::IsArray<TString>::value, const char*>::type
...
public:
    JsonParser(
        JsonBuffer* buffer, TReader reader, TWriter writer,
...
struct JsonParserBuilder<
    ...
    static TParser makeParser(
        TJsonBuffer* buffer, TChar* json, uint8_t nestingLimit) {
...
inline typename JsonParserBuilder<TJsonBuffer, TString>::TParser makeParser(
    TJsonBuffer* buffer, TString& json, uint8_t nestingLimit) {
    return JsonParserBuilder<TJsonBuffer, TString>::makeParser(
...
public:
    template <typename TString>
    typename Internals::EnableIf<
        !Internals::IsArray<TString>::value, JsonArray&>::type
    parseArray(
        const TString& json,
    ...
    template <typename TString>
    typename Internals::EnableIf<
        !Internals::IsArray<TString>::value, JsonObject&>::type
    parseObject(
        const TString& json,
        uint8_t nestingLimit = ARDUINOJSON_DEFAULT_NESTING_LIMIT) {
    ...
    template <typename TString>
    typename Internals::EnableIf<
        !Internals::IsArray<TString>::value, JsonVariant>::type
    parse(
        const TString& json,
        uint8_t nestingLimit = ARDUINOJSON_DEFAULT_NESTING_LIMIT) {
...
public:
    ...
    virtual void* alloc(size_t bytes) {
        alignNextAlloc();
        return canAllocInHead(bytes) ? allocInHead(bytes)
                                     : allocInNewBlock(bytes);
    }
    void clear() {
...
public:
    ...
    set(size_t index, T value, uint8_t decimals) {
        return set_impl<const JsonVariant&>(
            index, JsonVariant(value, decimals));
    }
    template <typename T>
    typename Internals::JsonVariantAs<T>::type get(size_t index) const {
        const_iterator it = begin() += index;
        return it != end() ? it->as<T>()
...
inline std::ostream& operator<<(
    std::ostream& os, const JsonArraySubscript& source) {
...
template <typename TStringRef>
inline std::ostream& operator<<(
    std::ostream& os, const JsonObjectSubscript<TStringRef>& source) {
    return source.printTo(os);
}
#endif
}  // namespace Internals
...
#define ARDUINOJSON_DEPRECATION_ERROR(X, Y)                    \
    ARDUINOJSON_COMPILE_ERROR(ARDUINOJSON_STRINGIFY(           \
        X is a Y from ArduinoJson 6 but version 5 is installed \
            .Visit arduinojson.org to get more information.))
#define StaticJsonDocument \
    ARDUINOJSON_DEPRECATION_ERROR(StaticJsonDocument, class)
#define DynamicJsonDocument \
    ARDUINOJSON_DEPRECATION_ERROR(DynamicJsonDocument, class)
#define JsonDocument ARDUINOJSON_DEPRECATION_ERROR(JsonDocument, class)
#define DeserializationError \
    ARDUINOJSON_DEPRECATION_ERROR(DeserializationError, class)
#define deserializeJson ARDUINOJSON_DEPRECATION_ERROR(deserializeJson, function)
#define deserializeMsgPack \
    ARDUINOJSON_DEPRECATION_ERROR(deserializeMsgPack, function)
#define serializeJson ARDUINOJSON_DEPRECATION_ERROR(serializeJson, function)
#define serializeMsgPack \
    ARDUINOJSON_DEPRECATION_ERROR(serializeMsgPack, function)
#define serializeJsonPretty \
    ARDUINOJSON_DEPRECATION_ERROR(serializeJsonPretty, function)
...
#define measureJsonPretty \
    ARDUINOJSON_DEPRECATION_ERROR(measureJsonPretty, function)
#endif
